...
func (r *WorkflowJobTemplatesNodeFailureResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_workflow_job_template_node_failure"
}
...
func (r *WorkflowJobTemplatesNodeFailureResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Specify a node ID and then a list of node IDs that should run when this one ends in failure.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"failure_ids": schema.SetAttribute{
				Required:    true,
				Description: "An unordered list of Node IDs attached to this workflow template node that should run on failure of this node.",
...
func (r *WorkflowJobTemplatesNodeFailureResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var data WorkflowJobTemplatesNodeFailureResourceModel
	...
	url := fmt.Sprintf("/api/controller/v2/workflow_job_template_nodes/%d/failure_nodes/", id)

	var relatedIds []int

	diags := data.FailureIds.ElementsAs(ctx, &relatedIds, false)
	if diags.HasError() {
		return
	}
...
func (r *WorkflowJobTemplatesNodeFailureResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	diags := data.FailureIds.ElementsAs(ctx, &PlanChildIds, false)
	if diags.HasError() {
		return
	}
